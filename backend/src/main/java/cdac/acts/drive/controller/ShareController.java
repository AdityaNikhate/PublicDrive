...
public class ShareController {
    ...
    public ResponseEntity<String> shareFile(@RequestBody SharedFileRequestDTO requestDTO) {
        ...
        // Find the user who is sharing the file by username (sharedByUsername)
        ...
        //Find the user to whom the file is being shared (sharedToUser)
        ...
        // Save the new SharedFile entry to the database
    ...
    public ResponseEntity<List<SharedFileResponseDTO>> getSharedFiles(@RequestParam String username) {
        ...
        // Find all shared files where the user is the sharedToUser
        List<SharedFile> sharedFiles = sharedFileRepository.findBySharedWithUser(sharedWithUser);

        // Convert the list of SharedFile entities into SharedFileResponseDTO
